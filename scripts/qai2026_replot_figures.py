...
def draw_gate(
    ...
    ax.add_patch(box)
    ax.text(x, y, label, ha="center", va="center", fontsize=fontsize, linespacing=1.05, zorder=5)


def draw_cx(ax: plt.Axes, x: float, y_control: float, y_target: float) -> None:
    ax.plot([x, x], [y_control, y_target], color="#263238", linewidth=1.1, zorder=3)
    ax.scatter([x], [y_control], s=24, color="#263238", zorder=5)
    target = patches.Circle((x, y_target), 0.022, facecolor="#fbfbfa", edgecolor="#263238", linewidth=1.0, zorder=5)
    ax.add_patch(target)
    ax.plot([x - 0.016, x + 0.016], [y_target, y_target], color="#263238", linewidth=0.9, zorder=6)
    ax.plot([x, x], [y_target - 0.016, y_target + 0.016], color="#263238", linewidth=0.9, zorder=6)


def draw_cz(ax: plt.Axes, x: float, y_a: float, y_b: float) -> None:
    ax.plot([x, x], [y_a, y_b], color="#263238", linewidth=1.1, zorder=3)
...
def draw_circuit_panel(
    ...
    ax.add_patch(panel)
    ax.text(0.05, 0.87, title, ha="left", va="center", fontsize=11, fontweight="bold", color="#263238")
